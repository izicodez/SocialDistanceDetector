...
######$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
                ...
                if (len(results) >= VIOLATION_COUNT_SETTING):#change this to family, university, work numbers
                    ...
                    if(stopwatch.elapsed >VIOLATION_TIME_SETTING and engine2):
                        ...
                        #
                        violation_count=len(results)
                        msg=str(f"{violation_count} violations are still detected in {ROOM_NAME} on {datee} for {timeCounter*2} Minutes. Please Check Immediately.")
...
if __name__ ==  '__main__': 
    ...
    while True:
        ...
        #str(f"yolo-coco{count}")
        labelsPath = os.path.sep.join([str("yolo-coco1"), "coco.names"])
        LABELS = open(labelsPath).read().strip().split("\n")
        print(str(f"yolo-coco{count}"))
    # derive the paths to the YOLO weights and model configuration
        weightsPath = os.path.sep.join(["yolo-coco1", "yolov3.weights"])
        configPath = os.path.sep.join([str("yolo-coco1"), "yolov3.cfg"])
    ...
    # determine only the *output* layer names that we need from YOLO
        ...
        thread = multiprocessing.Process(target=show_client, args=[addr, client_socket, lock])
        thread.start()
        #thread.join()
